...
create sequence seqTypePaiement start with 1 increment by 1;
...
alter table PaiementCommande add foreign key(typePaiement)references typePaiement(id);
...
create view VuePaiement AS
    SELECT pc.id,
        pc.nomclient,
        tr.nom AS tablerestau,
        pc.commande,
        pc.datepaiement,
        pc.montant,
        pc.retour,
        tp.id AS idtypepaiement,
        tp.nom AS nomtypepaiement
    FROM paiementcommande pc
        JOIN typepaiement tp ON pc.typepaiement = tp.id
        JOIN tablerestau tr ON pc.tablerestau = tr.id;
...
create view QuantiteUtilisationIngredient as
    SELECT '1' AS id,
        si.ingredient,
        i.nom,
        max(si.prix) AS prix,
    ...
    FROM enregistrementingredient si
        FULL JOIN ingredient i ON si.ingredient = i.id
    WHERE si.dateenregistrement >= '1900-01-01' AND si.dateenregistrement <= '2022-04-01'
    GROUP BY si.ingredient, i.nom;

create view QuantiteIngredient as
select
    si.ingredient,
    i.nom,
    sum(si.prix) as sommePrix,
    sum(si.quantite) as sommeQuantite
from
...
from
    (
        ...
        from
            ...
            Ingredient as i on si.ingredient=i.id
        where
            dateEntre between '1900/01/01' and '2021-04-22'
        group by
            si.ingredient,i.nom
    ) as qi join 
    (
        ...
        from
            ...
            Ingredient as i on si.ingredient=i.id
        where
            dateEnregistrement between '1900/01/01' and '2021-04-22'
        group by
            si.ingredient,i.nom
...
insert into TypePaiement values(nextVal('seqTypePaiement'),'Espece'),
(nextVal('seqTypePaiement'),'Cheque');

insert into PaiementCommande values(nextVal('seqPaiementCommande'),'1','Jean','1','1','2022/03/27 17:30:00',10000,0),
(nextVal('seqPaiementCommande'),'2','Anthoine','2','2','2022/03/27 10:10:10',3500,0),
(nextVal('seqPaiementCommande'),'1','Theo','3','3','2022/03/27 08:15:30',4500,0),
(nextVal('seqPaiementCommande'),'1','Paul','4','4','2022/03/27 15:10:30',8000,0),
(nextVal('seqPaiementCommande'),'1','Nick','5','5','2022/03/27 12:00:00',3500,0),
(nextVal('seqPaiementCommande'),'2','Andrew','6','6','2022/03/27 17:00:00',4000,0),
(nextVal('seqPaiementCommande'),'1','Tom','7','7','2022/03/27 16:30:00',3500,0);
